...
def kmeansApprox(xw,K,convergeDist):
    ...
    kPoints = xw.map(lambda k_w: k_w[0]).takeSample(False, K, 1)
    tempDist = 1.0
    while tempDist > convergeDist:
        ...
        closest = xw.map(lambda p_w: (weightedClosestPoint(p_w[0], p_w[1], kPoints)[0], (p_w[0], 1)))
        pointStats = closest.reduceByKey(lambda x1_y1, x2_y2: (x1_y1[0] + x2_y2[0], x1_y1[1] + x2_y2[1]))
        newPoints = pointStats.map(lambda x: (x[0], x[1][0] / x[1][1])).collect()
...
if __name__ == "__main__":
    ...
    Pcostsum = B.flatMap(lambda v: v[1]).reduce(add) #just pull out the costs to add
    ...
    Swithweights = PinS.zip(S.map(lambda ws: ws[1]))
    Bpts = B.map(lambda k: k[0])
    Bwithweights = Bpts.zip(S.map(lambda wb: wb[2]))
    ...
    while tempDist > convergeDist:
        ...
        pointStats = closest.reduceByKey(
            lambda x1_y1, x2_y2: (x1_y1[0] + x2_y2[0], x1_y1[1] + x2_y2[1]))
        newPoints = pointStats.map(
